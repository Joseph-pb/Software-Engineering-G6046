...
This section details the project timeline, task allocations and risk management strategies for the Cluedo/Clue! Desktop Game, developed in C\# / Unity.

\subsection{Initial Gantt Chart}

The project timeline and task assignments are visualised in the Gantt chart (Figure \ref{fig:gantt_chart}). This chart and the accompanying breakdown in Table \ref{tab:gantt_tasks} represent a combination of the individual Gantt charts developed at the start of each sprint, rather than an initial Gantt chart for the entire project. This approach was chosen as the problem was large and unfamiliar enough that we could not predict the scope from the beginning.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{gantt.png}
    \caption{Unity project timeline (19 Feb -- 30 Apr)}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{5.5cm}|p{1.8cm}|c|c|c|}
        \hline
        ...
        \texttt{UiManager}, \texttt{NotepadGrid} \& private modal & Matt & 17 Mar & 14 Apr & Done \\ \hline
        \texttt{RoomRules} (secret passages) \& \texttt{PlayTurn} coroutine & Ben, Ewan & 17 Mar & 14 Apr & Done \\ \hline
        \texttt{CharacterData} ScriptableObject \& JSON integration & Edward & 17 Mar & 14 Apr & Done \\ \hline
...
\begin{quote}
BoardScript → GameManager → Deck/Cards → TokenMovement → GameLoop → Suggestion/Accusation → Win condition
\end{quote}

Each task in this chain blocks the next. A player can only be placed once the board exists. They can only take a turn once GameManager exists. They can only move once Deck/Cards is set up (the deck is built during game initialisation). They can only make a Suggestion once they are physically in a room, which requires TokenMovement. Finally, they can only win via an Accusation evaluated against the solution envelope.
...
TokenMovement was the longest single task in the plan (17 Mar to 15 Apr, approximately 4 weeks) and had the highest downstream impact. Suggestion, GameLoop progression, and AI movement all depend on it. It was correctly identified as the highest-risk item, and in practice it did slip slightly past its planned end date.

\subsubsection*{Easter Break Disruption}
No development was scheduled or executed during the Easter holiday window (20 March to 14 April). Development was effectively paused for approximately 3 weeks, which compressed Sprint 3 work into the second half of April and forced parallel execution of TokenMovement, GameLoop, and Suggestion in the final 10 days.
...
\begin{itemize}
    \item Easter break paused development for approximately 3.5 weeks. There were no commits between 20 March and 14 April on any branch. All the work nominally scheduled across the second half of Sprint 3 was compressed into the period 15 to 24 April.
    \item TokenMovement slipped past its planned end date. The major movement-fix commits actually landed on 17 April and 18 April. Legal-move detection on the irregular Cluedo board required more edge-case handling than the plan accounted for.
    \item Suggestion and Accusation became a team task. Originally assigned to Ewan, the work was distributed across Edward (guessing logic), Ethan (looping mechanisms), and Ewan (GameStateScript and menus) under time pressure.
    \item Suggestion and Accusation logic merged into existing managers. In the delivered codebase, the logic lives inside \texttt{GameManagerScript.cs} and \texttt{UiManager.cs} rather than in standalone classes. Splitting the classes cleanly would have required a refactor the team did not have time for.
    \item The AI implementation was slightly reduced in complexity. The AI uses BFS-directed movement toward envelope-candidate rooms and knowledge-filtered suggestions. Full card deduction (eliminating based on inference) was not implemented.
    \item Sprint 2 absorbed unplanned repo infrastructure work. Significant effort went into Git LFS setup, \texttt{.gitignore} fixes, PR templates, and Unity merge settings. This was necessary to prevent merge conflicts in Unity scenes but consumed feature development time.
    \item Matt's script commits arrived late. Matt's earlier work was contributed via pair-programming or was scene and asset work in Unity (which does not surface as distinct script commits). His first direct script commits in the repository arrived on the 22nd of April.
\end{itemize}
...
Ethan & 7 & 12 & 12 & 18 & \textbf{49} \\ \hline
Ewan & 8 & 9 & 14 & 14 & \textbf{45} \\ \hline
Joe & 7 & 8 & 14 & 16 & \textbf{45} \\ \hline
Ben & 5 & 9 & 11 & 14 & \textbf{40} \\ \hline
...
\begin{tabular}{|c|p{2.8cm}|c|c|c|p{3cm}|c|}
\hline
\textbf{ID} & \textbf{Risk} & \textbf{L} & \textbf{I} & \textbf{Sev} & \textbf{Mitigation} & \textbf{Owner} \\ \hline
R1 & Unity version incompatibility & 4 & 3 & High & Agree LTS; test S1 & Matt \\ \hline
R2 & Git merge conflicts & 4 & 4 & High & YAML Merge; owner per scene & Edward \\ \hline
R3 & Member unavailable & 2 & 4 & Med & Pair-coding & All \\ \hline
R4 & Scope creep & 3 & 3 & Med & Prioritise core loop & Joe \\ \hline
R5 & Inconsistent dice & 3 & 2 & Med & Fallback logic & Ben \\ \hline
R6 & AI too complex & 3 & 2 & Low & Random moves first & Ben \\ \hline
R7 & Corrupted zip & 2 & 5 & Med & Verify unpacking & Joe \\ \hline
R8 & Incomplete loop & 2 & 5 & High & Prioritise playable & All \\ \hline
R9 & Knowledge gap & 5 & 4 & High & Tutorials; pair-code & All \\ \hline
\end{tabular}
...
\begin{enumerate}
    ...
    \item \textbf{Scope-reduction is a valid mitigation.} Cutting some final UI tweaks preserved the time for core features at the cost of not perfecting the UI design.
...
\begin{quote}
The marking criteria for \textit{Planning -- meeting notes} (band 5) requires:
...
\begin{itemize}
    ...
    \item \textbf{Actions Agreed} --- owner-tagged tasks, each with a \textit{Completed?} column so the reader can see whether the action was actually delivered (band 4).
\end{itemize}
...
\centering
\small
\begin{tabular}{|p{6cm}|p{1.8cm}|p{2.8cm}|}
\hline
...
\begin{itemize}
    \item\textbf{Code Documentation:} \texttt{C\#} XML doc comments agreed as the standard.
    \item\textbf{Privacy model discussed:} Options considered: (A) pass-and-play single screen, (B) cards hidden/revealed on demand via private modal, (C) split screen. Agreed upon Option B. Matt to implement a private modal in \texttt{UiManager} so revealed cards are shown only to the suggesting player.
    \item\textbf{Data-driven design:} Board layout, character names, weapon names loaded from JSON files using Unity's built-in JsonUtility.
\end{itemize}

\begin{table}[H]
\centering
\caption*{Role Assignments}
% Removed resizebox for better formatting
\begin{tabular}{|p{6cm}|p{1.8cm}|p{2.2cm}|}
\hline
...
Sprint 3 & AI \& polish & AI player agent, GUI polish, JSON loading \\ \hline
...
\hline
\textbf{Action} & \textbf{Owner} & \textbf{Completed?} \\ \hline
Refactor \texttt{DeckScript} so it initialises from GameManager & Edward & Yes (14/03) \\ \hline
...
\begin{itemize}
    ...
    \item Discussed AI --- agreed random legal moves are sufficient if we run out of time.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{AI movement:} BFS move selection is ideal, fallback to random if time is tight.
\end{itemize}
...
Wire AI move into \texttt{PlayTurn} coroutine & Ewan, Joe & Yes \\ \hline
...
\hline
\textbf{Action} & \textbf{Owner} & \textbf{Completed?} \\ \hline
Implement \texttt{RunSuggestion()} clockwise coroutine & Ewan & In progress \\ \hline
Implement accusation check vs \texttt{GetEnvelope()} & Ewan & In progress \\ \hline
AI suggestion when in room & Ben & In progress \\ \hline
...
Record gameplay video (MP4) & Ethan & Before 01 May\\ \hline
Final submission ZIP upload to Canvas & TBC & By 01 May\\ \hline
\end{tabular}
\end{table}
