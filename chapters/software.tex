...
\texttt{UIstuff.md} & UI development notes for menu / in-game panels \\ \hline
\texttt{matts-ui/} & Unity project root (open this folder in Unity Hub) \\ \hline
\texttt{matts-ui/Assets/Main Game/} & Game scene, prefabs, scripts \\ \hline
\texttt{matts-ui/Assets/Main Game/Scripts/} & 22 gameplay scripts (board,
deck, player, AI, UI manager, etc.) \\ \hline
\texttt{matts-ui/Assets/Main Game/Menu/} & 6 menu scripts (controller,
settings, stepper, modal, room plaque) \\ \hline
\texttt{matts-ui/Assets/Editor/} & Editor-only tooling
(\texttt{ProjectFileGenerator.cs}) \\ \hline
\texttt{matts-ui/Assets/Tests/EditMode/} & Sprint-aligned unit tests
(\texttt{Sprint1Tests.cs}\,--\,\texttt{Sprint4Tests.cs}) \\ \hline
\texttt{matts-ui/Assets/StreamingAssets/} & \texttt{cards.json} --- runtime
data contract for the deck \\ \hline
\texttt{report/} & All design / planning / testing / meeting documentation
sources (Markdown + PlantUML) \\ \hline
\texttt{report/overleaf/} & LaTeX source of this submission report \\ \hline
\end{tabular}
...
\begin{enumerate}
    ...
    \item In Unity Hub, click \textit{Add} $\to$ select the \texttt{matts-ui}
    folder.
    \item Open \textit{Assets / Main Game / Main Game} (the main scene) and
    press \textit{Play}.
    \item Configure \texttt{git config merge.unityyamlmerge.driver} once per
    machine using the OS-specific command in \texttt{README.md} ---
    this enables Unity Smart Merge for \texttt{.unity} and \texttt{.prefab}
    files (R2 mitigation, see \S\ref{sec:sprint2}).
\end{enumerate}
