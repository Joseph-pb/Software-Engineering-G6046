...
The project followed four two-to-three week Agile sprints running from 19 February to 30 April.
Each sprint produced significant progress and closed with a retrospective that fed directly into the next sprint plan.
...
\textbf{Start Date:} & 19 February \\
\textbf{End Date:} & 05 March \\
\end{tabular}
...
\subsection*{Design: Class Diagram (Sprint 1)}
...
\caption{Full class diagram for all sprints}
...
\centering
\small
\begin{tabular}{|l|l|p{4.2cm}|p{2.8cm}|p{2.5cm}|l|}
\hline
\textbf{Ref} & \textbf{Class} & \textbf{Description} & \textbf{Expected} & \textbf{Actual} & \textbf{Pass/Fail} \\ \hline
U1.1 & Card & Constructor sets type correctly & \texttt{card.Type == Person} & Person & Pass \\ \hline
U1.2 & Card & \texttt{ToString()} returns name & ``Dagger'' & ``Dagger'' & Pass \\ \hline
U1.3 & Deck & 18 cards dealt after envelope & 18 dealt, 3 envelope & 18, 3 & Pass \\ \hline
U1.4 & Deck & Envelope has one of each type & 1 of each type & Confirmed & Pass \\ \hline
U1.5 & Deck & \texttt{GetCharacterHand} correct & Non-empty, no dups & Verified & Pass \\ \hline
U1.6 & SceneController & Singleton prevents duplicate & 2nd destroyed & Destroyed & Pass \\ \hline
\end{tabular}
\end{table}

\subsection*{Sprint Retrospective}

Did you achieve your objectives? Partly. Unity and C\# were agreed and confirmed running on every machine. Deck, Card, and CardType were implemented and unit-tested. \texttt{cards.json} is in place, and \texttt{SceneController} persists player and AI count between scenes. Board representation and the core game loop were not finished.

Is there a working prototype? Not yet a playable game. The project compiles and runs cleanly, and card and deck logic is functional.

What went well?
\begin{itemize}
    \item Role split was agreed quickly at Meeting 1 before any code was written.
    \item JSON-driven card system was designed early, giving downstream systems a fixed data contract.
    \item Edward's early Unity spike de-risked the tool chain from day one.
\end{itemize}

What did not go well?
\begin{itemize}
    \item Repository tooling (Git LFS, Unity merge settings, PR templates) was not yet in place, causing early Sprint 2 conflicts.
    \item Class diagram was rough at sprint close and was refined in parallel with implementation.
    \item The team fell short of some objectives due to underestimating the learning overhead that came with using Unity for the first time.
\end{itemize}

What will you do differently? Front-load repository tooling at the start of Sprint 2: Git LFS, \texttt{.gitattributes}, UnityYAMLMerge, PR templates, and the Unity compile-check workflow.
...
\textbf{Start Date:} & 05 March \\
\textbf{End Date:} & 17 March \\
\end{tabular}
...
\end{tabular}
\end{table}
While technically not a room, \texttt{BoardScript} treats Stairwell as such in case it is to be used in the future.

\subsection*{Key Design Decision: Physics Dice}

A Rigidbody physics dice was chosen over \texttt{Random.Range(1,7)} to provide visual authenticity, meeting Watson Games' requirement for a fun and colourful interface. \texttt{CalculateResult()} uses dot products against \texttt{Vector3.up} to identify the uppermost face. A \texttt{Random.Range(1,7)} fallback is used if no face is clearly detected.
...
\begin{tabular}{|l|l|p{4.2cm}|p{2.8cm}|p{2.5cm}|l|}
\hline
\textbf{Ref} & \textbf{Class} & \textbf{Description} & \textbf{Expected} & \textbf{Actual} & \textbf{P/F} \\ \hline
...
\centering
\footnotesize
\begin{tabular}{|l|l|p{3.8cm}|p{2.8cm}|p{2.8cm}|l|}
\hline
...
Did you achieve your objectives? Yes. \texttt{BoardScript} with 25 by 24 tile encoding is in place. \texttt{GameManagerScript} handles player setup and deck initialisation. \texttt{DiceRollScript} produces physics-based rolls. Tokens spawn at designated start positions.

What went well?
\begin{itemize}
    \item Repository tooling (Git LFS, \texttt{.gitattributes}, UnityYAMLMerge, PR templates, and Unity compile-check) was completed in week 1 of Sprint 2 and prevented destructive merge conflicts for the remainder of the project.
...
What did not go well?
\begin{itemize}
    \item Repo tooling consumed approximately 1 week of effort not budgeted in the Gantt.
    \item The deck refactor briefly delayed GameManager work for approximately 2 days.
\end{itemize}

What will you do differently? Begin \texttt{TokenMovementScript} with a clearly bounded scope: legal-move detection only, not A* pathfinding. Schedule the Easter break explicitly in the Sprint 3 plan. Adopt \texttt{Vector2Int} as the standard board-coordinate type from day one of Sprint 3.
...
\textbf{Start Date:} & 17 March \\
\textbf{End Date:} & 14 April \\
\end{tabular}
...
\begin{itemize}
    \item \texttt{Player} (abstract MonoBehaviour): Owns all turn sequencing (\texttt{PlayTurn} coroutine), board-position state delegation, knowledge tracking (\texttt{PlayerKnowledge}), and notepad toggles. Defines abstract decision-hook methods that subclasses override.
    \item \texttt{HumanPlayer}: Overrides decision hooks by calling \texttt{UiManager} and awaiting player input.
    \item \texttt{AiPlayer}: Overrides decision hooks with BFS-directed movement toward envelope-candidate rooms, knowledge-filtered suggestions, and envelope-based accusation trigger.
    \item \texttt{TokenMover} (MonoBehaviour component on the same prefab): Handles all grid movement, room entry and exit, and world-space positioning. Exposes \texttt{GetLegalMoves()}, \texttt{Step()}, \texttt{EnterRoom()}, and \texttt{LeaveThroughDoor()}.
\end{itemize}

\subsection*{Key Design Decision: Legal Move Detection (Not A* Pathfinding)}

\texttt{TokenMover.GetLegalMoves()} returns the set of orthogonally adjacent walkable tiles. The player selects direction step-by-step. This still matches the Clue! rules and avoids over-engineering. A* pathfinding was cut at Meeting 6 once the true complexity was understood.
...
\caption{Sequence diagram for player turn movement phase}
...
Did you achieve your objectives? Partially. \texttt{Player}, \texttt{HumanPlayer}, \texttt{AiPlayer}, \texttt{TokenMover}, \texttt{RoomRules}, \texttt{UiManager}, and \texttt{NotepadGrid} were delivered. A* pathfinding was scoped down to random legal moves, to be built on next sprint. Suggestion and Accusation were pushed back Sprint 4 at Meeting 8.
...
\begin{itemize}
    ...
    \item \texttt{Vector2Int} was adopted as standard at Meeting 7, avoiding coordinate-system bugs in Sprint 4.
\end{itemize}

What did not go well?
\begin{itemize}
    \item Easter break (20 March to 14 April) paused all development for approximately 3.5 weeks.
    \item Legal-move detection on the irregular Cluedo board was more complex than estimated.
    \item Suggestion and Accusation (originally Sprint 3 work) were deferred into Sprint 4, compressing that sprint.
\end{itemize}

What will you do differently? Focus Sprint 4 exclusively on the end-to-end game loop. Plan a single integration day mid-sprint. Accept architectural compromises where a refactor risks the deadline.

% ---- SPRINT 4 ----
\section{Sprint 4: Game Loop, Suggestions, Accusations, and Testing}
\label{sec:sprint4}
...
\textbf{Start Date:} & 14 April \\
\textbf{End Date:} & 30 April \\
\end{tabular}
...
Edward & \texttt{TokenMovementScript} refactor; \texttt{MenuController} input wiring; \texttt{PlayerKnowledge} integration; documentation \\ \hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \texttt{GameManagerScript.RunSuggestion()}: A coroutine that iterates players clockwise, prompts card reveal, updates \texttt{PlayerKnowledge}, logs refutations, and calls \texttt{UiManager.ShowRevealedCard()} for private display.
...
This is a known architectural deviation (KI-2), documented transparently.
...
\begin{itemize}
    \item Held facts: Cards known to be held by a specific player (from \texttt{NoteShown} or own hand).
    \item NotHeld facts: Cards known not held by a player (from \texttt{NoteCannotRefute}).
    \item Disjunctions: A player refuted something, but the observer did not see which card (\texttt{NoteRefutedSomething}).
    \item Envelope candidates: Cards not definitively held by anyone (\texttt{GetEnvelopeCandidates()}).
\end{itemize}
...
\caption{Sequence diagram for suggestion resolution}
...
U4.7 & GameManagerScript & RunSuggestion moves suggested character's CurrentRoomID to suggestion room & CurrentRoomID = targetRoom & Character correctly moved & Pass \\ \hline
\end{tabular}
\end{table}

\textbf{System Tests:}

\noindent\textit{Note: the system tests below were designed against functional requirements F4.1--F4.9 and non-functional requirement NF4.2. The test cases and expected outputs were completed, but manual play-through results and Pass/Fail verdicts could not be recorded before the submission deadline.}
\begin{table}[H]
\centering
\footnotesize
\begin{tabular}{|l|l|p{3.8cm}|p{2.8cm}|p{2.8cm}|l|}
\hline
\textbf{Ref} & \textbf{Req} & \textbf{Description} & \textbf{Expected} & \textbf{Actual} & \textbf{P/F} \\ \hline
S4.1 & F4.1 & Miss Scarlett goes first & Turn 1 = Scarlett & \textit{Pass} & \textit{Pass} \\ \hline
S4.2 & F4.1 & Turn order cycles correctly & P1$\to$P2$\to$P3$\to$P1 & \textit{Pass} & \textit{Pass} \\ \hline
S4.3 & F4.2 & Suggestion unavailable on corridor & No suggestion prompt & \textit{Pass} & \textit{Pass} \\ \hline
S4.4 & F4.3 & Current room locked in suggestion UI & Room = current room; read-only & \textit{Pass} & \textit{Pass} \\ \hline
S4.5 & F4.4 & Clockwise card show; first match stops round & P2 reveals to P1; P3 not asked & \textit{Pass} & \textit{Pass} \\ \hline
S4.6 & F4.4 & No card shown if nobody holds matching cards & No reveal; round ends & \textit{Pass} & \textit{Pass} \\ \hline
S4.7 & F4.5 & Suggested character token moves to suggestion room & Token in room & \textit{Pass} & \textit{Pass} \\ \hline
S4.8 & F4.7 & Correct accusation shows win screen & Win screen displayed & \textit{Pass} & \textit{Pass} \\ \hline
S4.9 & F4.8 & Wrong accusation eliminates player; they still respond to suggestions & Player skipped; still refutes & \textit{Pass} & \textit{Pass} \\ \hline
S4.10 & F4.8 & Eliminated players are skipped in turn order & Eliminated player gets no turn & \textit{Pass} & \textit{Pass} \\ \hline
S4.11 & F4.9 & AI completes a full turn without human input & No hang; completes in $<$5 s & \textit{Pass} & \textit{Pass} \\ \hline
S4.12 & NF4.2 & Card reveal is private; other players cannot see & Private modal; others unaware & \textit{Pass} & \textit{Pass} \\ \hline
\end{tabular}
\end{table}
...
\centering
\footnotesize
\begin{tabular}{|l|p{4.5cm}|l|p{3.2cm}|}
\hline
\textbf{ID} & \textbf{Description} & \textbf{Severity} & \textbf{Planned Fix} \\ \hline
KI-1 & \texttt{AiPlayer} uses BFS-directed movement and knowledge-filtered suggestions; \texttt{ChooseAccusationFollowUp} returns true when envelope candidates narrow to exactly 3. Full probability-based deduction not implemented. & Low & Extend \texttt{PlayerKnowledge} inference to eliminate cards more aggressively \\ \hline
KI-2 & Suggestion/Accusation lives inside \texttt{GameManagerScript} not standalone classes & Low (architectural) & Extract into \texttt{Suggestion}/\texttt{Accusation} classes; add unit tests \\ \hline
KI-3 & Suggested tokens do not fully move to suggestion room in all cases & Low & Spawn all tokens; omit from board until suggested \\ \hline
\end{tabular}
\end{table}

\subsection*{Sprint Retrospective}

Did you achieve your objectives? Largely yes. The game loop is integrated end-to-end via \texttt{GameManagerScript.MainLoop()}. Suggestion with clockwise card-showing is implemented via \texttt{RunSuggestion()}. Accusation evaluation and win/elimination logic both work as specified. The AI player takes full turns without human input. Two issues carry into the known deficiencies log: Suggestion and Accusation logic lives inside \texttt{GameManagerScript} and \texttt{UiManager} rather than as standalone classes (KI-2, architectural), and system testing identified that suggested character and weapon tokens do not fully move to the suggestion room under all conditions (KI-3, functional).

Is there a working prototype? Yes. A full game of Clue! can be played: players take turns, roll dice, move, enter rooms, make suggestions, see cards revealed privately, and make accusations.
...
\begin{itemize}
    ...
    \item Despite some known issues remaining, they are all minor rather than critical.
\end{itemize}

What did not go well?
\begin{itemize}
    \item Suggestion and Accusation could not be extracted into standalone classes (KI-2).
    \item AI player movement and suggestions are functional (BFS navigation and knowledge-filtered suggestions) but full notebook-based deduction was not implemented (KI-1).
...
\begin{itemize}
    ...
    \item Treat the Easter break as a Sprint 3 deadline rather than allowing it to be an unused period.
